...
def get_correct_letter():
    q = db_get_questions()[current_question_index]
...
async def broadcast_leaderboard():
    await broadcast({"type": "leaderboard", "players": players})

async def broadcast_question():
    await broadcast({"type": "question", "data": get_current_question(), "duration": QUESTION_DURATION})

async def broadcast_answer_stats():
    await broadcast({"type": "answer_stats", "counts": answer_counts, "correct_answer": get_correct_letter()})

async def close_question():
    global question_open
    question_open = False
    await broadcast({"type": "question_closed", "correct_answer": get_correct_letter()})
...
@app.get("/api/config")
def api_config():
    return {"room_code": ROOM_CODE, "question_duration": QUESTION_DURATION}

@app.get("/api/questions")
def api_list_questions():
    rows = db_get_questions()
    return {"items": [
        {"id": q.id, "question": q.question, "options": [q.option_a, q.option_b, q.option_c, q.option_d], "correct": q.correct}
        for q in rows
    ]}
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        await websocket.send_text(json.dumps({"type": "room_info", "room_code": ROOM_CODE, "quiz_started": quiz_started, "question_open": question_open, "question_duration": QUESTION_DURATION}))
        await websocket.send_text(json.dumps({"type": "leaderboard", "players": players}))
        if quiz_started and get_question_count() > 0:
            await websocket.send_text(json.dumps({"type": "question", "data": get_current_question(), "duration": QUESTION_DURATION}))
        while True:
            ...
            if msg_type == "join":
                ...
                if room_code != ROOM_CODE:
                    await websocket.send_text(json.dumps({"type": "info", "message": "Oda kodu yanlış."}))
                    continue
                if not name:
                ...
                if current_question_index < total - 1:
                    ...
                    await broadcast({"type": "quiz_finished"})
                    await broadcast_leaderboard()
                ...
                await broadcast({"type": "info", "message": "Quiz sıfırlandı."})
                await broadcast_leaderboard()
                ...
                await websocket.send_text(json.dumps({"type": "answer_result", "correct": is_correct, "your_answer": answer, "correct_answer": correct, "score": players[player_name]}))
                await broadcast_leaderboard()
                await broadcast({"type": "host_answer_info", "player": player_name, "answer": answer, "correct": is_correct})
